...
/* === HUMAN RETURN ENGINE JUMP FIX v1 === */
/* This block fixes protocol card clicking even if older handlers fail. */
...
(function () {
  ...
  function hardJumpProtocol(key) {
    const data = HRE_PROTOCOLS[key];
    if (!data) return;

    document.querySelectorAll(".protocol").forEach(card => {
      card.classList.toggle("active", card.dataset.protocol === key);
    });

    setText("protocolStage", data.stage);
    setText("protocolTitle", data.title);
    setText("protocolDesc", data.desc);
    setText("protocolDescCN", data.descCN);
    setText("metricBpm", data.bpm);
    setText("metricTarget", data.target);
    setText("metricExit", data.exit);
    setText("miniProtocol", data.title);
    setText("qdrOutput", data.qdr);
    setText("orbText", data.orb);

    document.querySelectorAll(".route span").forEach(span => {
      span.classList.toggle("active-step", span.textContent.trim() === data.title);
    });

    window.HRE_currentProtocol = key;
  }

  window.HRE_jumpProtocol = hardJumpProtocol;

  function bindProtocolClicks() {
    document.querySelectorAll(".protocol").forEach(card => {
      ...
      card.addEventListener("click", function (event) {
        ...
        hardJumpProtocol(card.dataset.protocol);
      }, true);
    ...
    if (grid) {
      grid.addEventListener("click", function (event) {
        const card = event.target.closest(".protocol");
        if (!card) return;
        event.preventDefault();
        event.stopPropagation();
        hardJumpProtocol(card.dataset.protocol);
    ...
    hardJumpProtocol("breakbeats");
  }

  if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", bindProtocolClicks);
  } else {
    bindProtocolClicks();
  }
})();
